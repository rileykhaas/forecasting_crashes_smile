...
constituents' lower bounds (its Figure~10). But an average of individual crash
probabilities is \emph{not} the probability the sector crashes: a diversified sector
can be calm while its members are each risky. Our sector-ETF surface prices the
sector's crash probability \emph{directly}. Figure~\ref{fig:industrycompare} puts the
two on the same axes for the six Fama-French industries with a clean Select Sector
SPDR counterpart, assigning each S\&P~500 constituent to an FF12 industry from its
CRSP SIC code.
...
The proxy overstates it in \emph{every} sector (the Proxy$-$Real.\ column is positive
throughout, by up to nine points), confirming that an average of individually risky
names is not the diversified sector's crash probability. The direct measure is far
better calibrated for the broad, well-diversified sectors --- essentially on top of
realized for financials ($+0.007$) and manufacturing ($+0.000$), and closest in health
care, where individually risky biotech names keep the proxy elevated at $0.11$ against
a direct bound a quarter of that. For the higher-beta, less internally diversified
sleeves (energy, technology, utilities) the direct bound sits \emph{below} the
realized frequency: as a \emph{lower} bound it stays conservative where a sector's
crashes are driven by idiosyncratic sector shocks (oil, rates, the tech cycle) rather
than the market --- exactly where the comonotonicity that makes the bound tight is
least applicable. In every case, though, the direct measure is the sector's own crash
probability; the proxy is not.
...
sector-ETF lower bound (blue), one-month 20\% crash probability, 1996--2022, for the
six Fama-French industries with a clean SPDR sector counterpart. The proxy lies above
the direct measure in every panel: averaging individually risky names overstates the
diversified sector's crash risk. Health care is the clearest case --- a persistently
elevated proxy against a near-zero direct bound. The takeaway: an average of
constituent crash probabilities is not the sector's crash probability, and the ETF
surface supplies the latter directly.}
...
the realized crash frequency (12-month horizon, $q=0.80$, 1996--2022). ``Proxy'' is
the mean equal-weighted constituent lower bound (the paper's Figure-10 measure);
...
measure's gap to it (its tightness). The takeaway: the proxy overstates the realized
frequency in every sector, while the direct measure tracks it for the broad
diversified sectors and stays conservative (below) for the higher-beta ones.}
\label{tab:industry}
