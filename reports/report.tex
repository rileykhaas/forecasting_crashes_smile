...
estimate through five steps. (1) \emph{Risk-neutral density}: price out-of-the-money
options on a fine moneyness grid, differentiate (Breeden--Litzenberger) to recover
the risk-neutral CDF of the gross return, and enforce a valid distribution with an
isotonic fit. (2) \emph{Risk-neutral crash probability}: read $P^{*}[R \le q]$
directly from that CDF. (3) \emph{Fear correction}: reweight by a power-utility
investor's marginal utility ($\gamma = 2$, calibrated by the paper on the market)
to move toward the physical measure. (4) \emph{Copula bounds}: close the unknown
stock--market dependence with Fr\'echet--Hoeffding bounds; the comonotone
\emph{lower bound} is the forecaster. (5) \emph{Calibration test}: regress the
realized crash indicator on each probability --- the theory predicts a slope of
one for the lower bound, not zero.
...
reader meets the argument: first the raw signal for individual names and for the
market (Figures~1 and 2), then the pooled summary statistics (Table~1) and the
formal calibration test (Table~2).

\paragraph{Figure 1 --- bounds for individual stocks.} Figure~\ref{fig:fig1} plots
the time series of the option-implied upper and lower bounds on the one-month
probability of a $20\%$ crash for two representative names, Apple and AIG. It shows
how the bounds move with market stress, and how far apart the ``cries wolf''
risk-neutral reading and the fear-corrected lower bound can be for a single name.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{\PathToOutput/fig1_single_name_bounds.png}
\caption{Option-implied bounds on the probability of a $20\%$ crash over one month
for AIG (red/pink) and Apple (blue/light-blue), 1996--2022; the shaded band spans
each name's lower and upper bound. The bounds vary sharply across firms and over
time --- AIG's crash risk spikes during the 2008--09 financial crisis, exactly when
the uncorrected risk-neutral signal ``cries wolf'' loudest, while Apple's is
elevated in the early-2000s tech unwind.}
\label{fig:fig1}
\end{figure}

\paragraph{Figure 2 --- the market and the cross-section.}
Figure~\ref{fig:fig2} plots the cross-sectional medians of the upper and lower
bounds over time, together with the crash probability of the S\&P~500 index itself
(the $i = m$ case of Result~3). It lifts the single-name picture to the aggregate
and shows that the market crash probability is lower and smoother than the typical
constituent's.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{\PathToOutput/fig2_median_bounds_market.png}
\caption{Cross-sectional median across S\&P~500 constituents of the upper (red) and
lower (blue) option-implied bounds on the probability of a $20\%$ crash over one
month, with the S\&P~500 index's own crash probability (grey, the $i = m$ lower
bound), 1996--2022. The median bounds spike in every crisis, but the \emph{market}
crash probability is consistently lower and smoother than the typical constituent's
--- individual stocks are riskier than the diversified index.}
\label{fig:fig2}
\end{figure}

\paragraph{Table 1 --- summary statistics.} Table~\ref{tab:table1} reports, for
each crash size $q$ and horizon $\tau$, the realized crash frequency, our lower and
upper bounds, and the risk-neutral probability, averaged both across firms (a
monthly cross-section) and across time (per firm). The headline is visible in every
panel: the mean lower bound sits next to the mean realized crash frequency, whereas
the risk-neutral probability and the upper bound run well above both. Our
cross-sectional means match the paper to within $0.007$.

\begin{table}[htbp]
\centering
\small
\caption{Summary statistics --- replication of Martin \& Shi (2025), Table~1,
over January 1996--December 2022. For each crash size $q$ (a gross return at or
below $q$) and horizon $\tau$, we report the realized crash frequency, the
option-implied lower and upper bounds, and the risk-neutral probability. The
\emph{lower bound tracks the realized frequency} while the risk-neutral
probability and the upper bound overstate crash risk --- the paper's central
finding. $T$ is the number of formation months; $N$ the number of firms.}
\label{tab:table1}
\input{\PathToOutput/table1.tex}
\end{table}

\paragraph{Table 2 --- calibration.} Table~2 regresses the realized crash
indicator on each probability measure. A well-calibrated forecast has a slope of
one: the lower bound delivers $\beta \approx 1$, whereas the risk-neutral
probability is badly miscalibrated (slope well below one). This is the formal
counterpart of Table~1's visual message.
\pending{Table 2 --- regressions of realized crashes on the lower/upper bounds and the risk-neutral probability}
