...
Applying the bound to the eleven Select Sector SPDR ETFs (plus KRE, regional banks)
yields a \emph{direct} per-sector crash-probability series, in contrast to the
paper's average-of-constituents proxy. This is not a weakening of the theory but a
clean test of it: the comonotonicity assumption that makes the lower bound tight is
far more plausible for a diversified sector ETF than for a single name, so the bound
should bind \emph{more} tightly here. And an average of individual crash
probabilities is not the probability the sector crashes --- the ETF surface gives the
sector's crash probability directly. The ETFs run through exactly the same engine as
the stocks (they are spot-matched in cleaning and priced in the pipeline once their
permnos are pulled), so no new estimation is introduced.

Figure~\ref{fig:etf} plots each sector's direct lower-bound one-month 20\% crash
probability against the S\&P~500's own (the $i=m$ benchmark). The measure behaves
exactly as economic intuition demands: technology (XLK) spikes through the 2000--02
dot-com unwind, financials (XLF) and regional banks (KRE) in 2008--09, energy (XLE)
in the 2014--16 oil collapse, while defensive sectors (staples XLP, utilities XLU,
health care XLV) stay quiet and below the index. Table~\ref{tab:etf} states the
benchmark numerically: for the diversified broad sectors the mean lower bound sits
close to the realized crash frequency (financials $0.079$ vs.\ $0.073$; the index
itself $0.055$ vs.\ $0.068$), while the risk-neutral probability again runs two to
three times too high --- the paper's ``crying wolf'' result, now confirmed at the
sector level. The bound is more conservative for the less-diversified,
higher-beta sleeves (energy, technology, and especially regional banks), consistent
with comonotonicity binding tightest exactly where diversification is greatest.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{\PathToOutput/fig_etf_sector_bounds.png}
\caption{Direct sector-ETF lower-bound probability of a 20\% one-month crash (blue),
each against the S\&P~500 index's own crash probability (grey, the $i=m$ bound),
1996--2022. Each sector's option surface prices its \emph{own} crash risk directly,
rather than averaging its constituents: technology leads in 2000--02, financials and
regional banks in 2008--09, energy in 2014--16, while defensive sectors stay below
the index throughout. The takeaway: the theory-based measure produces an economically
coherent, sector-specific crash gauge, tightest where the market-comonotonicity
assumption is most plausible.}
\label{fig:etf}
\end{figure}
...
\centering
\small
\caption{Sector-ETF crash-probability bounds vs.\ the realized frequency (12-month
horizon, $q=0.80$, 1996--2022). $\overline{P^L}$, $\overline{P^*}$, $\overline{P^U}$
are the mean lower bound, risk-neutral probability, and upper bound; ``Realized'' is
the realized frequency of a 20\% crash; $N$ counts the months with an observed
outcome (shorter for the later-launched ETFs, XLRE and XLC). The takeaway: for the
diversified broad sectors the lower bound hugs the realized frequency while the
risk-neutral overstates it two- to three-fold, reproducing the paper's core finding
directly at the sector level; the bound is more conservative for the concentrated,
higher-beta sleeves.}
\label{tab:etf}
...
constituents' lower bounds, on the Fama--French 49-industry classification (its
Figure~10). But an average of individual crash probabilities is \emph{not} the
probability the sector crashes: a diversified sector can be calm while its members are
each risky. Our sector-ETF surface prices the sector's crash probability
\emph{directly}. Figure~\ref{fig:industrycompare} puts the two on the same axes for
\emph{all eleven} Select Sector SPDR sectors, using the paper's Figure-10 horizon (the
one-year probability of a 20\% crash). We reproduce the paper's FF49 assignment ---
each S\&P~500 constituent gets its FF49 industry from its CRSP SIC code --- and roll
the FF49 industries up to the SPDR sector they compose. Five sectors map cleanly
(financials, technology, health care, energy, utilities); the other six, marked with an
asterisk, are best-fit approximations because FF49's manufacturing and consumer
industries straddle the GICS sector lines. One deliberate refinement: FF49 files REITs
under ``Trading'', but they are the GICS Real Estate sector and the bulk of XLRE, so we
route them to Real Estate (which also de-contaminates the post-2016 financials sleeve).

The pattern is consistent: the proxy (orange) sits \emph{above} the direct bound
(blue) in every sector and at almost every date --- averaging risky names
systematically overstates the diversified sector's crash probability.
Table~\ref{tab:industry} measures each against the sector's realized crash frequency.
The proxy overstates it in every sector but one (the Proxy$-$Real.\ column is positive
throughout, by up to nine or ten points), confirming that an average of individually
risky names is not the diversified sector's crash probability. The direct measure is
far better calibrated for the broad, well-diversified sectors --- essentially on top of
realized for financials ($+0.007$) and industrials ($+0.000$), and starkest in health
care and real estate, where individually risky biotech names and single REITs keep the
proxy above $0.11$ against a direct bound less than half that. For the higher-beta,
less internally diversified sleeves (energy, technology, utilities) the direct bound
sits \emph{below} the realized frequency: as a \emph{lower} bound it stays conservative
where a sector's crashes are driven by idiosyncratic sector shocks (oil, rates, the
tech cycle) rather than the market --- exactly where the comonotonicity that makes the
bound tight is least applicable. The lone exception is communication services, whose
XLC realized column is inflated by a short, crash-heavy post-2018 sample (the sector
ETF launched in 2018, as did XLRE); read those two realized figures with that caveat.
In every case, though, the direct measure is the sector's own crash probability; the
proxy is not.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{\PathToOutput/fig_industry_compare.png}
\caption{The paper's average-of-constituents proxy (orange) vs.\ our direct
sector-ETF lower bound (blue), one-year 20\% crash probability, 1996--2022, for all
eleven Select Sector SPDR sectors. FF49 industries are rolled up to their SPDR sector;
an asterisk marks the six best-fit (non-clean) mappings. The proxy lies above the
direct measure in every panel: averaging individually risky names overstates the
diversified sector's crash risk. Health care and real estate are the clearest cases ---
a persistently elevated proxy against a much lower direct bound. XLC and XLRE launched
in 2018, so their direct series are shorter. The takeaway: an average of constituent
crash probabilities is not the sector's crash probability, and the ETF surface supplies
the latter directly.}
...
\centering
\small
\caption{Proxy vs.\ direct sector-ETF lower bound, and each measure's distance from
the realized crash frequency (one-year horizon, $q=0.80$, 1996--2022), for all eleven
SPDR sectors. ``Proxy'' is the mean equal-weighted constituent lower bound (the paper's
Figure-10 FF49 measure); ``Direct'' is the sector ETF's own mean lower bound;
``Realized'' is the sector's realized 20\% crash frequency; ``Proxy$-$Real.'' and
``Direct$-$Real.'' are each measure's gap to it (its tightness). An asterisk marks the
six sectors whose FF49$\to$GICS mapping is a best-fit approximation. The takeaway: the
proxy overstates the realized frequency in every sector (bar the short-sample XLC),
while the direct measure tracks it for the broad diversified sectors and stays
conservative (below) for the higher-beta ones.}
\label{tab:industry}
\resizebox{\textwidth}{!}{\input{\PathToOutput/industry_tightness.tex}}
\end{table}

\subsection{Silicon Valley Bank: one event at three levels}
As a final stress test, we run the machinery at \emph{daily} frequency through the
March~2023 collapse of Silicon Valley Bank, reading one event at three levels: the
failed name (SIVB), the regional-bank ETF (KRE), and broad financials (XLF). The same
engine prices each daily surface; nothing else changes. This is the bound at its
structural weak point --- theory says the lower bound is tight under market
comonotonicity but conservative for an \emph{idiosyncratic} crash, where the
risk-neutral probability, carrying no fear premium to remove, is the better estimate.
SVB is exactly that scenario, with a known resolution: the bank failed, the index did
not.

Figure~\ref{fig:svb} shows the result, and it reads as a real-time classifier of
whether the market saw the event as contained or systemic. At the name level, SIVB's
crash probability is quiet through February, drifts up in early March, and then
explodes to $36\%$ (risk-neutral) on 9~March --- the last day it traded, after which
its option surface simply ends, the market having stopped pricing it as a going
concern. The stress then \emph{decays sharply} across the three levels: the
regional-bank ETF (KRE) jumps from near zero to a $17.6\%$ peak on 17~March, while
broad financials (XLF) reach only $6.1\%$. That ordering --- $36\% \to 18\% \to 6\%$
--- is the market pricing the event as concentrated in the name and its sector rather
than systemic to the financial system, exactly the classification the theory's three
levels are meant to separate. Consistent with the idiosyncratic-crash prediction, the
risk-neutral probability sits above the lower bound throughout the name's collapse;
for SIVB, the risk-neutral is the measure to watch.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{\PathToOutput/fig_svb_case_study.png}
\caption{The March~2023 SVB collapse read at three levels, daily: the risk-neutral
probability (red) and option-implied lower bound (blue) of a 20\% one-month crash for
SIVB (the failed bank), KRE (regional banks), and XLF (broad financials). SIVB's crash
probability explodes to $36\%$ on 9~March and its surface then ends as the bank fails;
the stress decays across the levels ($36\% \to 18\% \to 6\%$), the market pricing the
event as contained to the name and its sector rather than systemic. For an
idiosyncratic crash the risk-neutral sits above the (conservative) lower bound, as the
theory predicts.}
\label{fig:svb}
\end{figure}
...
\centering
\small
\caption{Crash probability vs.\ realized outcome for the SVB episode. ``Peak $P^*$''
and ``Peak $P^L$'' are the maximum risk-neutral and lower-bound 20\% one-month crash
probabilities during Feb--Mar 2023; ``Realized drawdown'' is the return from the
8~March close to the March trough. SIVB was closed by regulators on 10~March and its
equity effectively wiped, so its $-60\%$ understates the true loss. The peak
probabilities rank-order the realized drawdowns and the 20\% line separates crash from
no-crash --- the measure classified the name, its sector, and the system correctly.}
\label{tab:svb}
...
manufacturing $0.067$ vs.\ $0.066$) and staying conservative, as a lower bound should,
