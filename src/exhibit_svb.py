...
from rates import build_rates  # noqa: E402
from run_pipeline import run_pipeline  # noqa: E402
from pull_svb_daily import CASE_LABELS, SIVB_SECID, KRE_SECID, XLF_SECID  # noqa: E402
...
PANELS = [(SIVB_SECID, "SIVB — SVB Financial (the failed name)"),
...
def daily_clean_surface(raw_surface, spot_daily):
    ...
    df = df[(df["impl_strike"] > 0) & (df["dispersion"] > 0) & (df["dispersion"] < 0.05)]
    ...
    n_strikes = df.groupby(["date", "secid", "days_to_maturity"])["impl_strike"].transform("nunique")
    df = df[n_strikes > 10]
    out = df[["date", "secid", "days_to_maturity", "moneyness", "implied_vol",
              "spot_price", "cp_flag"]].copy()
    out["secid"] = out["secid"].astype("int64")
    out["days_to_maturity"] = out["days_to_maturity"].astype("int64")
    return out.sort_values(["date", "secid", "days_to_maturity", "moneyness"]).reset_index(drop=True)
...
def _panel_series(bounds, secid, threshold_q, horizon_days):
    ...
    d = bounds[(bounds["secid"] == secid) & (bounds["threshold_q"] == threshold_q)
               & (bounds["horizon_months"] == months)]
    return d.sort_values("date")


def svb_realized_table(bounds, spot_daily, threshold_q=THRESHOLD_Q,
                       horizon_days=HORIZON_DAYS, ref_date=REF_DATE):
    ...
    for secid, _ in PANELS:
        ...
        ps = (spot_daily[spot_daily["secid"] == secid]
              .sort_values("date").set_index("date")["spot_price"])
        ...
        rows.append(dict(
            ticker=CASE_LABELS[secid], level=LEVEL[secid],
...
def build_figure_svb(bounds, threshold_q=THRESHOLD_Q, horizon_days=HORIZON_DAYS,
                     title=TITLE):
    ...
    for ax, (secid, label) in zip(axes, PANELS):
        s = _panel_series(bounds, secid, threshold_q, horizon_days)
        ax.plot(s["date"], 100 * s["prob_riskneutral"], color=C_RN, lw=1.6,
                marker="o", ms=3, label=r"Risk-neutral $P^*$")
        ax.plot(s["date"], 100 * s["bound_lower"], color=C_LB, lw=1.6,
                marker="o", ms=3, label=r"Lower bound $P^L$")
        ax.axvline(COLLAPSE_DATE, color="#333333", lw=1.0, ls="--")
        top = ax.get_ylim()[1]
        ax.text(COLLAPSE_DATE, top * 0.97, " SVB fails (Mar 10)", fontsize=8,
...
if __name__ == "__main__":
    ...
    for secid, label in PANELS:
        ...
        if len(s):
            i = s["prob_riskneutral"].idxmax()
            print(f"{CASE_LABELS[secid]}: P* peak {100*s.loc[i,'prob_riskneutral']:.1f}% "
