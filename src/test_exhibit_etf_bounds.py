    ...
    for date in ["2010-01-31", "2010-02-28"]:
        for secid, lo, rn, up, flag in spec:
            rows.append(dict(date=pd.Timestamp(date), secid=secid, threshold_q=0.80,
                             horizon_months=12, bound_lower=lo, prob_riskneutral=rn,
                             bound_upper=up, realized_flag=flag))
    # a decoy q/horizon that must be ignored
    rows.append(dict(date=pd.Timestamp("2010-01-31"), secid=110012, threshold_q=0.70,
                     horizon_months=1, bound_lower=0.99, prob_riskneutral=0.99,
                     bound_upper=0.99, realized_flag=1))
