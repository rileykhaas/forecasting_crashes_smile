...
def _raw(secid=110012, date="2023-03-09", days=30, n=12, dispersion=0.01, strike0=90.0):
    ...
    return pd.DataFrame({
        "secid": secid, "date": pd.Timestamp(date), "days": days, "cp_flag": "C",
        "delta": 0.5, "impl_volatility": 0.3,
        "impl_strike": [strike0 + i for i in range(n)], "dispersion": dispersion,
    })
...
def test_daily_clean_uses_exact_date_spot():
    ...
    raw = pd.concat([_raw(date="2023-03-08", strike0=100.0),
                     _raw(date="2023-03-09", strike0=100.0)])
    spot = pd.DataFrame({
        "secid": [110012, 110012],
...
def test_daily_clean_applies_appendix_d_filters():
    ...
    spot = pd.DataFrame({"secid": [1, 2, 3], "date": [pd.Timestamp("2023-03-09")] * 3,
                         "spot_price": [100.0] * 3})
...
def test_empty_realized_is_typed_and_empty():
    er = _empty_realized()
    assert len(er) == 0
    assert list(er.columns) == ["date", "secid", "horizon_months", "realized_gross_return"]


def test_realized_table_flags_crash_by_drawdown():
    """A 20% threshold: a -25% drawdown is a crash, a -10% one is not."""
    bounds = pd.DataFrame({  # peak P* for one secid at q=0.80, 30-day
        "date": pd.to_datetime(["2023-03-09", "2023-03-09"]),
        "secid": [SIVB_SECID, XLF_SECID], "threshold_q": [0.80, 0.80],
        "horizon_months": [1, 1], "bound_lower": [0.30, 0.04],
        "prob_riskneutral": [0.35, 0.06], "bound_upper": [0.4, 0.1],
    })
    spot = pd.DataFrame({
        "secid": [SIVB_SECID, SIVB_SECID, XLF_SECID, XLF_SECID],
        "date": pd.to_datetime(["2023-03-08", "2023-03-20", "2023-03-08", "2023-03-20"]),
...
@pytest.mark.skipif(not (DATA_DIR / "svb_daily_surface.parquet").exists(),
...
def test_real_svb_story_holds():
    ...
    from pull_svb_daily import load_svb_surface, load_svb_spot, load_svb_zero, KRE_SECID
    ...
    assert peak(SIVB_SECID) > peak(KRE_SECID) > peak(XLF_SECID)  # contained, not systemic
