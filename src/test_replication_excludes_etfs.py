...
import schema
from exhibit_eda import coverage_by_year, constituent_frame
from exhibit_fig2 import cross_sectional_medians
from exhibit_table1 import build_table1

ETF_SECIDS = {110007, 110008, 110009, 110010, 110011, 110012,
              110013, 110014, 110015, 127104, 208181, 213084}
    ...
    for date in ["2010-01-31", "2010-02-28"]:
        for secid, lo, hi in [(1, 0.02, 0.06), (2, 0.04, 0.10), (110012, 0.99, 0.99)]:
            rows.append(dict(date=pd.Timestamp(date), secid=secid, threshold_q=0.80,
                             horizon_months=1, bound_lower=lo, prob_riskneutral=0.5,
                             bound_upper=hi, realized_gross_return=1.0,
                             realized_flag=0))
...
def test_fig2_medians_ignore_etf_rows():
    ...
    med = cross_sectional_medians(_results_with_etf(), _member_panel(),
                                  start=pd.Timestamp("2010-01-01"), end=pd.Timestamp("2010-12-31"))
...
def test_table1_excludes_etf_rows():
    ...
    stats = build_table1(_results_with_etf(), _member_panel(),
                         start=pd.Timestamp("2010-01-01"), end=pd.Timestamp("2010-12-31"))
    lb = stats[(stats["q"] == 0.80) & (stats["measure"] == "lower bound")
               & (stats["block"] == "firms") & (stats["horizon"] == 1)]["mean"].iloc[0]
...
def test_eda_constituent_frame_drops_etfs():
    ...
    surface = pd.DataFrame({
        ...
        "secid": [1, 2, 110012],
        "days_to_maturity": [30, 30, 30], "moneyness": [1.0, 1.0, 1.0],
        "implied_vol": [0.2, 0.2, 0.2], "spot_price": [100.0, 100.0, 100.0],
        "cp_flag": ["C", "C", "C"],
    })
...
@pytest.mark.skipif(not (DATA_DIR / "sp500_secid_universe.parquet").exists(),
